---
layout: archive
title: "Lab Members"
permalink: /members/
author_profile: false
---
<h2>Principal Investigator</h2>
<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Yanjun Su.png" alt="Yanjun Su" sizes="max-width: 600px">
  </div>
  <div style="margin-left: 20px;">
    <h2>Yanjun Su</h2>
    <p>Professor, Institute of Botany, Chinese Academy of Sciences<br>Email: <a href="mailto:ysu@ibcas.ac.cn">ysu@ibcas.ac.cn</a></p>
    <p>Address: No. 20, Nanxin Village, Xiangshan, Haidian District, Beijing, CA: 100093</p>
    <p><strong>Research interests:</strong> His research interests lie in the use of lidar as well as other remote sensing techniques to quantify forest canopy structure, and investigate how anthropogenic activities and climatic changes influence the terrestrial ecosystem processes. He currently serves as an associate editor for Plant Phenomics and the Geoscience Data Journal, and he reviews for a number of journals, including Nature Cities, Remote Sensing of Environment, New Phytologist, Ecology, and the ISPRS Journal of Photogrammetry and Remote Sensing.</p>
  </div><br>
</div>
<hr>

<h2>Staffs</h2>
<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Tianyu Hu.png" alt="Tianyu Hu" width: 400px; height: 200px>
  </div>
  <div style="margin-left: 20px;">
    <h2>Tianyu Hu</h2>
    <p>Associate Professor, Institute of Botany, Chinese Academy of Sciences<br>Email: <a href="mailto:tianyuhu@ibcas.ac.cn">tianyuhu@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research focuses on using light detection and ranging (LiDAR) technology and dynamic vegetation model to understand forest ecosystem, especially in forest structure, function, and biodiversity.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Xiaoyong Wu.png" alt="Xiaoyong Wu" height: 200px>
  </div>
  <div style="margin-left: 20px;">
    <h2>Xiaoyong Wu</h2>
    <p>Ph.D Candidate<br>Email: <a href="mailto:xiaoyongwu@ibcas.ac.cn">xiaoyongwu@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research centers on employing remote sensing techniques to uncover the interaction mechanisms among “canopy structure-climate factors-forest ecosystem processes” within the context of climate change.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Ru Xu.png" alt="Ru Xu" width=400px height=200px>
  </div>
  <div style="margin-left: 20px;">
    <h2>Ru Xu</h2>
    <p>Research Associate, Institute of Botany, Chinese Academy of Sciences<br>Email: <a href="mailto:xuru@ibcas.ac.cn">xuru@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> Her research interests encompass a wide range of areas, including remote sensing mapping of land cover changes, the role of forests in climate regulation, and the application of Earth system models.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Xiaoqiang Liu.png" alt="Xiaoqiang Liu" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Xiaoqiang Liu</h2>
    <p>Postdoctoal, Institute of Botany, Chinese Academy of Sciences<br>Email: <a href="mailto:liuxiaoqiang@ibcas.ac.cn">liuxiaoqiang@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research interests include using light detection and ranging (lidar) and machine learning to quantify forest canopy structure, and investigating how climatic changes and anthropogenic activities influence forest ecosystem functions.</p>
  </div><br>
</div>
<hr>

<h2>Students</h2>
<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Xiaoyong Wu.png" alt="Xiaoyong Wu" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Xiaoyong Wu</h2>
    <p>Ph.D Candidate<br>Email: <a href="mailto:xiaoyongwu@ibcas.ac.cn">xiaoyongwu@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research centers on employing remote sensing techniques to uncover the interaction mechanisms among “canopy structure-climate factors-forest ecosystem processes” within the context of climate change.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Jiatong Wang.png" alt="Jiatong Wang" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Jiatong Wang</h2>
    <p>Ph.D Student<br>Email: <a href="mailto:wangjiatong@ibcas.ac.cn">wangjiatong@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research interests lie in using Lidar data to quantify the fine structure of tree branches and understand its spatial variation.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Yilin Long.png" alt="Yilin Long" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Yilin Long</h2>
    <p>Ph.D Student<br>Email: <a href="mailto:longyilin21@mails.ucas.ac.cn">longyilin21@mails.ucas.ac.cn</a></p>
    <p><strong>Research interests:</strong> Her research primarily revolves around Dynamic Global Vegetation Models (DGVMs), lies in exploring the complexities of how different ecosystems respond to and influence climate change.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Liwen Huang.png" alt="Liwen Huang" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Liwen Huang</h2>
    <p>Ph.D Student<br>Email: <a href="mailto:huangliwen23@mails.ucas.ac.cn">huangliwen23@mails.ucas.ac.cn</a></p>
    <p><strong>Research interests:</strong> Her research interests lie in using remote sensing techniques to identify the forest dominant species and explore the forest edge effect.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Xu Yan.png" alt="Xu Yan" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Xu Yan</h2>
    <p>Ph.D Student<br>Email: <a href="mailto:yanxu8121@163.com">yanxu8121@163.com</a></p>
    <p><strong>Research interests:</strong> His undergraduate research focused on global meta-analysis of nitrous oxide emission from soil of agroforestry system.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Yue Luo.png" alt="Yue Luo" height: 200px>
  </div>
  <div style="margin-left: 20px;">
    <h2>Yue Luo</h2>
    <p>M.D Student<br>Email: <a href="mailto:luoyue@ibcas.ac.cn">luoyue@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> During his master's program, his primary research focus is large-scale, high-resolution data products.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Qi Dong.png" alt="Qi Dong " style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Qi Dong </h2>
    <p>M.D Student<br>Email: <a href="mailto:dongqi23@mails.ucas.ac.cn">dongqi23@mails.ucas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research interests lie in using Lidar data to understand relationship between tree health and environment.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Jiabo Yan.png" alt="Jiabo Yan" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Jiabo Yan</h2>
    <p>M.D Student<br>Email: <a href="mailto:yanjiabo23@mails.ucas.ac.cn">yanjiabo23@mails.ucas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research interests lie in developing algorithms for separation the single-tree wood and leaf from terrestrial Lidar point cloud.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Xuanhao Jiang.png" alt="Xuanhao Jiang" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Xuanhao Jiang</h2>
    <p>M.D Student<br>Email: <a href="jiangxuanhao@ibcas.ac.cn">jiangxuanhao@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His undergraduate research focuses on the long-term estimation of grassland fractional vegetation cover in China through combination of massive drone images and satellite data.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Haoen Xu.png" alt="Haoen Xu" style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Haoen Xu</h2>
    <p>M.D Student<br>Email: <a href="mailto:xuhaoen@ibcas.ac.cn">xuhaoen@ibcas.ac.cn</a></p>
    <p><strong>Research interests:</strong> His research interests lie in the interactions between organism and abiotic factors, utilizing satellite and geostatistics methods, computer modelling, unmanned aerial vehicles (UAVs) technology, and light detection and ranging (LiDAR) technology.</p>
  </div><br>
</div>
<hr>

<div style="display: flex; align-items: center;">
  <div>
    <img src="/images/Zhichao Wei.png" alt="Zhichao Wei " style="width: 200px; height: 200px;">
  </div>
  <div style="margin-left: 20px;">
    <h2>Zhichao Wei </h2>
    <p>M.D Student<br>Email: <a href="mailto:weipepino@outlook.com">weipepino@outlook.com</a></p>
    <p><strong>Research interests:</strong> His research interests lie in the investigation of natural grassland plant diseases in several counties of Gansu Province, and the study of seed germination and seedling growth characteristics of common grasses on the Qinghai-Tibet Plateau.</p>
  </div><br>
</div>
<hr>

<h2>Alumni</h2>
<p>Mr. Xiaotian Qi&emsp;&emsp;&emsp;&emsp;&thinsp;Huawei Technologies Co., Ltd<br>
Dr. Xiaoxia Zhao&emsp;&emsp;&emsp;&ensp;Inner Mongolia Academy of Science and Technology<br>
Dr. Chunyue Niu&emsp;&emsp;&emsp;&ensp;China Academy of Space Technology<br>
Dr. Yuhao Feng&emsp;&emsp;&emsp;&emsp;The University of Hong Kong<br>
Dr. Qin Ma&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;Huzhou Municipal Bureau of Culture, Radio, TV and Tourism<br>
Miss. Mengqi Cao&emsp;&emsp;&emsp;University of Helsinki, Finland<br>
Mr. Zhonghua Liu&emsp;&emsp;&emsp;The University of Hong Kong<br>
Miss. Xuejing Wang&emsp;&thinsp;&ensp;&ensp;Henan Province Meteorologic Administration<br>
Dr. Qiuli Yang&emsp;&emsp;&emsp;&emsp;&emsp;Xinjiang University<br>
Miss. Mohan Yang&emsp;&emsp;&emsp;Longhua District, Shenzhen Affiliated schools of the Academy of Educational Sciences</p>
